...
interface User {
  id: number
  name: string
  team_id: number | null // team_id can be null if user is not in a team
  team_name: string | null // team_name can be null if user is not in a team
  points: number
  email?: string
}

interface Team {
  id: number // Add id to Team interface to enable deletion
  name: string
  points: number; // Changed from 'score' to 'points' to match scoreboard's ApiTeam structure
  country: string
}
...
export default function AdminDashboard() {
  ...
  const fetchData = async (token: string) => {
    try {
      ...
      // Ensure teamData.teams is an array and each team object has a 'points' property
      const teamData: { teams: { id: number; name: string; points: number; country: string }[] } = await teamRes.json();
      const challData = await challRes.json()
      const notificationData = await notificationRes.json()

      setUsers(userData.users || [])
      setTeams(teamData.teams || []) // This should now correctly map to the 'points' property
  ...
  const handleUpdateChallenge = async (e: FormEvent) => {
    ...
    if (!editingFlag.trim()) {
      console.log("A new flag is required to update a challenge."); // Using console.log instead of alert
      return;
    }
  ...
  const handleCreateNotification = async (e: FormEvent) => {
    ...
    try {
      ...
      await fetchData(token);
      console.log('Notification sent successfully!'); // Replaced alert
    } catch (error) {
      console.error('Error creating notification:', error);
      console.log(`Error: ${error}`); // Replaced alert
    }
  };
  ...
  const handleUpdateNotification = async (e: FormEvent) => {
    ...
    try {
      ...
      await fetchData(token);
      console.log('Notification updated successfully!'); // Replaced alert
    } catch (error) {
  ...
  const handleDeleteNotification = async (notificationId: number) => {
    ...
    try {
      ...
      await fetchData(token);
      console.log('Notification deleted successfully!'); // Replaced alert
    } catch (error) {
  ...
  // --- Event Handlers for User and Team Deletion ---

  const handleDeleteUser = async (userId: number, userName: string) => {
    const token = localStorage.getItem('token');
    if (!token) return;

    if (!window.confirm(`Are you sure you want to ban/delete user "${userName}"? This action cannot be undone.`)) {
      return;
    }

    try {
      const res = await fetch(`http://localhost:8000/user/delete/${userId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(errorData.detail || 'User deletion failed');
      }

      console.log(`User "${userName}" deleted successfully!`); // Replaced alert
      await fetchData(token); // Re-fetch data to update the UI
    } catch (error) {
      console.error('Error deleting user:', error);
      console.log(`Error deleting user: ${error}`); // Replaced alert
    }
  };
  ...
  const handleDeleteTeam = async (teamId: number, teamName: string) => {
    ...
    if (!window.confirm(`Are you sure you want to delete team "${teamName}"? This will also affect its members and solves.`)) {
      return;
    }

    try {
      const res = await fetch(`http://localhost:8000/team/delete/${teamId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(errorData.detail || 'Team deletion failed');
      }

      console.log(`Team "${teamName}" deleted successfully!`); // Replaced alert
      await fetchData(token); // Re-fetch data to update the UI
    } catch (error) {
      console.error('Error deleting team:', error);
      console.log(`Error deleting team: ${error}`); // Replaced alert
    }
  };
  ...
  return (
    <div className="bg-[#1d1a29] text-white min-h-screen p-6">
      ...
      <section className="mb-10">
        ...
        <table className="w-full bg-[#e0f7fa] text-black rounded shadow-md">
          <thead className="bg-blue-800 text-white">
            <tr><th>Team</th><th>Total Points</th><th>Country</th><th>Actions</th></tr>
          </thead>
          <tbody>
            {teams.map((t, i) => (
