...
export default function AdminDashboard() {
    ...
    // --- NEW: Event Handlers for User and Team Deletion ---

    const handleDeleteUser = async (userId: number, userName: string) => {
        const token = localStorage.getItem('token');
        if (!token) return;

        if (!confirm(`Are you sure you want to ban/delete user "${userName}"? This action cannot be undone.`)) {
            return;
        }
    ...
    const handleDeleteTeam = async (teamId: number, teamName: string) => {
        ...
        if (!confirm(`Are you sure you want to delete team "${teamName}"? This will also affect its members and solves.`)) {
            return;
        }
                        ...
                        {users.map((u, i) => (
                          ...
                          suppressHydrationWarning
                            className="text-center border-b hover:bg-blue-100 transition text-lg font-['Jaini_Purva']">
                                <td>{u.name}</td>
                                <td>{u.points}</td>
                                <td>{u.team_name || 'N/A'}</td> {/* Display 'N/A' if team_name is null */}
                                <td className="text-lg">
                                    {/* Edit button could open a modal for user editing */}
                                    <button
                                        onClick={() => alert(`Edit functionality for user ${u.name} not yet implemented`)}
                    ...
                    <tbody>
                        {teams.map((t, i) => (
                            <tr key={i} className="text-center text-lg border-b hover:bg-blue-100 font-['Jaini_Purva'] transition">
                                ...
                                <td>{t.country}</td>
                                <td className="text-lg">
                                    {/* Edit button could open a modal for team editing */}
                                    <button
                                        onClick={() => alert(`Edit functionality for team ${t.name} not yet implemented`)}
                                        className="text-blue-600 cursor-pointer hover:underline hover:text-blue-800 transition mr-2"
                                    >
                                        Edit
                                    </button>
